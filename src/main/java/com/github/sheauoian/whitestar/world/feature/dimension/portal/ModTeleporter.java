package com.github.sheauoian.whitestar.world.feature.dimension.portal;
...
public class ModTeleporter implements ITeleporter {
    ...
    public Optional<BlockUtil.FoundRectangle> getExistingPortal(BlockPos pos) {
        ...
        Optional<PoiRecord> optional = poiManager.getInSquare((poiType) ->
                poiType.get() == ModPOIs.DONFAI_PORTAL.get(), pos, 64, PoiManager.Occupancy.ANY).sorted(Comparator.<PoiRecord>comparingDouble((poi) ->
            ...
            for(int l1 = -1; l1 < 2; ++l1) {
                for(int k2 = 0; k2 < 2; ++k2) {
                    for(int i3 = -1; i3 < 3; ++i3) {
                        BlockState blockstate1 = i3 < 0 ? ModBlocks.WHITE_PLANKS.get().defaultBlockState() : Blocks.AIR.defaultBlockState();
        ...
        for(int k1 = -1; k1 < 3; ++k1) {
            for(int i2 = -1; i2 < 4; ++i2) {
                if (k1 == -1 || k1 == 2 || i2 == -1 || i2 == 3) {
                    ...
                    this.level.setBlock(mutablePos, ModBlocks.WHITE_PLANKS.get().defaultBlockState(), 3);
                }
            }
        }

        BlockState DonfaiPortal = ModBlocks.DONFAI_PORTAL.get().defaultBlockState().setValue(DFPortalBlock.AXIS, axis);

        for(int j2 = 0; j2 < 2; ++j2) {
            for(int l2 = 0; l2 < 3; ++l2) {
                mutablePos.setWithOffset(blockpos, j2 * direction.getStepX(), l2, j2 * direction.getStepZ());
                this.level.setBlock(mutablePos, DonfaiPortal, 18);
            }
        }
    ...
    public PortalInfo getPortalInfo(Entity entity, ServerLevel level, Function<ServerLevel, PortalInfo> defaultPortalInfo) {
        boolean destinationIsUG = level.dimension() == ModDimensions.DONFAIDIM_KEY;
        if (entity.level.dimension() != ModDimensions.DONFAIDIM_KEY && !destinationIsUG) {
            return null;
        }
        else {
            ...
            return this.getOrMakePortal(entity, blockpos).map((result) -> {
                BlockState blockstate = entity.level.getBlockState(entity.getOnPos()); //TODO
                Direction.Axis axis;
                Vec3 vector3d;
                if (blockstate.hasProperty(BlockStateProperties.HORIZONTAL_AXIS)) {
                    axis = blockstate.getValue(BlockStateProperties.HORIZONTAL_AXIS);
                    BlockUtil.FoundRectangle rectangle = BlockUtil.getLargestRectangleAround(entity.getOnPos(), axis, 21, Direction.Axis.Y, 21, (pos) -> entity.level.getBlockState(pos) == blockstate);
    ...
    protected Optional<BlockUtil.FoundRectangle> getOrMakePortal(Entity entity, BlockPos pos) {
        ...
        if(existingPortal.isPresent()) {
            ...
            Direction.Axis portalAxis = this.level.getBlockState(entity.getOnPos()).getOptionalValue(DFPortalBlock.AXIS).orElse(Direction.Axis.X);
